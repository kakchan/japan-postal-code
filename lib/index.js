...
var getWithFilter = exports.getWithFilter = function getWithFilter(_postCode, maxItems) {
  ...
  return (0, _nodeFetch2.default)(postCodeFileUrl(fileName), {
    ...
    var _loop = function _loop(postcode) {
      if (new RegExp(postCode).test(postcode)) {
        var newItems = json[postcode].map(function (item) {
          return item.concat(postcode);
        });
    ...
    for (var postcode in json) {
      var _ret = _loop(postcode);
